...
{
  internal partial class WindowRosterRealization : WindowRoster.Interface
  {
    #region Properties

    public string Title { get; private set; } = "Ship Manifest Roster";
    internal float WindowWidth = 700;
    internal float WindowHeight = 330;
    public Rect Position { get; set; } = new Rect(0, 0, 0, 0);
    internal Rect ViewBox = new Rect(0, 0, 680, 230);
    public bool ShowWindow { get; set; } 
    public string ToolTip { get; set; } = "";
    internal bool ToolTipActive;
    public bool ShowToolTips {get; set; } = true;
    internal float XOffset = 30;
    internal float YOffset = 90;

    // RosterList must exist outside of the vessel.
    internal List<ProtoCrewMember> RosterList = new List<ProtoCrewMember>();

    internal Professions KerbalProfession;

    // Gender var
    internal ProtoCrewMember.Gender Gender = ProtoCrewMember.Gender.Male;
    internal KerbalFilters CurrentFilter = KerbalFilters.All;

    public bool OnCreate { get; set; }

    private Vector2 _scrollViewerPosition = Vector2.zero;

    public bool ResetRosterSize
    {
      get
      {
    ...
    private ModKerbalRealization _selectedKerbal;
    public ModKerbal SelectedKerbal
    {
      get { return _selectedKerbal; }
      set
      {
        _selectedKerbal = (ModKerbalRealization)value;
    ...
    #region Gui Layout
    public void Display(int windowId)
    {
      ...
      try
      {
        ...
        {
          Rect p = Position;  // Hacks, hacks, hacks everywhere!
          SMAddon.RepositionWindow(ref p);
          Position = p;
        }
      }
      catch (Exception ex)
    ...
    private bool CanDisplayKerbal(ProtoCrewMember kerbal)
    {
    ...
    private void CreateKerbalViewer()
    {
      ...
      if (GUILayout.Button(guilabel, GUILayout.MaxWidth(80), GUILayout.Height(20)))
      {
        ...
        while (!kerbalFound)
        {
          SelectedKerbal = ModKerbalRealization.CreateKerbal(kerbalType);
    ...
    private void DisplaySelectProfession()
    {
      GUILayout.BeginHorizontal();
    ...
    private void DisplayRosterFilter()
    {
      GUILayout.BeginHorizontal();
    ...
    private void DisplayRosterListViewer()
    {
      try
      {
        GUILayout.BeginVertical();
        ...
        while (kerbals.MoveNext())
        {
            ...
            GUILayout.Height(20)))
          {
            ...
            {
              SelectedKerbal = ModKerbalRealization.Create(kerbals.Current, false);
              SetProfessionFlag();
            }
            else
            {
    ...
    private void SetupEditButton(ProtoCrewMember kerbal, out string buttonText, out string buttonToolTip)
    {
    ...
    private void SetProfessionFlag()
    {
    ...
    public void ThawKerbal(string kerbalName)
    {
      try
      {
